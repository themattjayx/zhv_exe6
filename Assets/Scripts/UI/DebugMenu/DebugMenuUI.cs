...
#region Internal
    ...
    private static Vector2 WINDOW_DIMENSION = new Vector2(400.0f, 300.0f);
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                */
                ...
                {
                    if (GUILayout.Button("Cheat 100 coins"))                              //added button
                    {
                        InventoryManager.Instance.availableCurrency = 100;
                    }
                    GUILayout.Label("Currency: ", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                    GUILayout.Label($"{InventoryManager.Instance.availableCurrency}", GUILayout.Width(WINDOW_DIMENSION.x / 8.0f));
                }
                ...
                GUILayout.Label($"Screen dimensions:{Screen.width}, {Screen.height} ", GUILayout.Width(WINDOW_DIMENSION.x / 1.0f));
                /*
                 *
                ...
                GUILayout.BeginVertical();
                {
                    if (GUILayout.Button("Enable Interactive Mode"))
                    {
                    ...
                    //used audio listener because of additional sounds
                    var volume = AudioListener.volume;
                    volume = (float) GUILayout.HorizontalSlider(volume, 0.0f, 1.0f, GUILayout.ExpandWidth(true));
                    if (GUI.changed)
                    {
                ...
                GUILayout.Label("Time speed: ", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    { // Create a set of 10 sliders all sharing the same value.
                        ...
                        if (iii == 10)
                        {
                            Time.timeScale = mDummyValue * 0.1f + 1.0f;
                        }
                        //added time speed (time scale) based on last slider
                    }
